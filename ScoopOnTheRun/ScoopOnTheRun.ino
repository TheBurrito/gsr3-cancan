...
bool checkCanScoop = false;  // flag to check if we just drove through a can and have it in the grippers
...
void loop() {
  ...
  if (irbFL or irbFR && mode != mDriveGoal && mode != mDropCan && mode != mBackup) {
    checkCanScoop = true;
  }

  if (irbF && mode != mGrabCan && mode != mDriveGoal && mode != mDropCan && mode != mBackup) {
    //    checkCanScoop = true;
  }
  ...
  if (checkCanScoop) {
    if (millis() - lastGrip >= gripDelay) {
      ...
      if (curGrip <= SERVO_G_CLOSE) {
        ...
        if (irbF) { // check if a can is in the gripper
          mode = mDriveGoal;  
          checkCanScoop = false;
        }
        else {
  ...
  case mDriveGoal:
    ...
    else if (!irbF) {  // don't go empty handed to the goal
      if (errorCnt = errorThresh) {
        RobotBase.stop();
        errorCnt = 0;
        curGrip = SERVO_G_OPEN;
