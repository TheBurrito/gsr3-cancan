...
void CRobotBase::setVelocityRange(double maxVel, double deadVel,
        double minVel) {
    _maxVel = maxVel;
    _deadVel = deadVel;
    _minVel = minVel;
...
void CRobotBase::setLocalization(const double& x, const double& y,
        const double& theta) {
    _fixX = x - _posX;
    _fixY = y - _posY;
    _fixTheta = theta - _theta;
...
void CRobotBase::updateOdometry(const long& encL, const long& encR,
        const double& dt) {
...
void CRobotBase::updateVelocity(double targetVel, double targetTurn,
        const double& dt) {
#if DEBUG_VELOCITY
    Serial.print("Trgt Vel: ");
    Serial.print(targetVel);
    Serial.print("  Min Vel: ");
    Serial.print(_minVel);
#endif
#if DEBUG_TURNING
    Serial.print("Trgt Turn: ");
    Serial.print(targetTurn);
    Serial.print("  Turn Adj: ");
    Serial.print(_turnAdjust);
    Serial.print("  Min Turn: ");
    Serial.println(_minTurn);
#endif
    ...
    if (absVel < _deadVel)
        targetVel = 0;
    else if (absVel < _minVel)
        targetVel = _minVel * signVel;
    else if (absVel > _maxVel)
        targetVel = _maxVel * signVel;

    if (absTurn < _deadTurn)
        targetTurn = 0;
    else if (absTurn < _minTurn)
        targetTurn = _minTurn * signTurn;
    else if (absTurn > _maxTurn)
        targetTurn = _maxTurn * signTurn;

#if DEBUG_VELOCITY
    Serial.print("Abs Trgt Vel: ");
    Serial.println(targetVel);
#endif
#if DEBUG_TURNING
    Serial.print("Abs Trgt Turn: ");
    ...
    if (leftVel > _curVelL + accel)
        leftVel = _curVelL + accel;
    else if (leftVel < _curVelL - accel)
        leftVel = _curVelL - accel;

    if (abs(leftVel) < accel)
        leftVel = 0;
    if (abs(rightVel) < accel)
        rightVel = 0;

    if (rightVel > _curVelR + accel)
        rightVel = _curVelR + accel;
    else if (rightVel < _curVelR - accel)
        rightVel = _curVelR - accel;
    ...
    if (absL < _deadOut)
        _left = 0;
    else if (absL < _minOut)
        _left = _minOut * signL;
    else if (absL > _maxOut)
        _left = _maxOut * signL;

    if (absR < _deadOut)
        _right = 0;
    else if (absR < _minOut)
        _right = _minOut * signR;
    else if (absR > _maxOut)
        _right = _maxOut * signR;
...
int CRobotBase::readIR(IR_Index ir) {
    ...
    case IR_1080:
        ...
        if (_irSamples > 2) {
            ...
            puntualDistance = sum / _irSamples;

        if (puntualDistance > 80)
            puntualDistance = 81;
        if (puntualDistance < 10)
            puntualDistance = 9;
        break;
    ...
    case IR_20150:
        ...
        if (puntualDistance > 150)
            puntualDistance = 151;
        if (puntualDistance < 20)
            puntualDistance = 19;
        break;
    }
...
void CRobotBase::setNavThresh(const double& posThresh,
        const double& thetaThresh) {
