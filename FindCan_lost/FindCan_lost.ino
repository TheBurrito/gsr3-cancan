...
void detectCan(int sensor, int curDist) {
  ...
  if (curDist > min && curDist < max) {  // ignore values outside the sensors specs
    ...
    float thetaD = RobotBase.getTheta() * 180 / PI;
    float sensX = sensXrel * cos(thetaD) - sensYrel * sin(thetaD);
    float sensY = sensXrel * sin(thetaD) - sensYrel * cos(thetaD);
    float objX = curDist * (cos(sensors[sensor].angle) ) + sensX;
    float objY = curDist * (sin(sensors[sensor].angle) ) + sensY;
//    float objX = sensX + curDist * cos(RobotBase.getTheta() * 180 / PI);
//    float objY = sensY + curDist * sin(RobotBase.getTheta() * 180 / PI);
//    float objX = curDist * (cos(sensors[sensor].angle + RobotBase.getTheta() * 180 / PI)) + sensors[sensor].offset.x + RobotBase.getX();
//    float objY = curDist * (sin(sensors[sensor].angle + RobotBase.getTheta() * 180 / PI)) + sensors[sensor].offset.y + RobotBase.getY();
//    objX = curDist * (cos(sensors[sensor].angle)+ RobotBase.getTheta()) + sensors[sensor].offset.x + RobotBase.getX();
//    objY = curDist * (sin(sensors[sensor].angle) + RobotBase.getTheta()) + sensors[sensor].offset.y + RobotBase.getY();
