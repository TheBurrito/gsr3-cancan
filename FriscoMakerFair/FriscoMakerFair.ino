...
#define MAX_X 3 * 12 * 2.54 - WALL_BUFFER - ROBOT_FRONT_OFFSET - ROBOT_GRIP_OFFSET
#define MAX_Y 6 / 2 * 12 * 2.54 - WALL_BUFFER - ROBOT_FRONT_OFFSET - ROBOT_GRIP_OFFSET
#define MIN_X 0 
...
*/
  ...
  // X < min : X = min
  // X > max : x = max
  RobotBase.setOutputRange(300, 5, 60);
...
//  if (mode != mWander) return;
  if (sensor == IRF || sensor == IRL || sensor == IRR) return;  // don't try to detect cans by width with the front sensor
