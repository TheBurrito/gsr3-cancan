...
TimedAction chooseCanAction = TimedAction(5000,chooseCan);
...
*/
  ...
  // X < min : X = min
  // X > max : x = max
  RobotBase.setOutputRange(340, 5, 50);
  ...
  RobotBase.setIRFilter(0.8);
  RobotBase.setIRSamples(2);
...
void loop() {
  ...
  if (irbF && curGrip == SERVO_G_CLOSE && mode != mDriveGoal) {
    mode = mDriveGoal;
  }
  else if (!irbF && !irbFL && !irbFR && curGrip == SERVO_G_CLOSE) {
    if (millis() - lastGrip >= gripDelay) {
      curGrip += gripStep;
      if (curGrip >= SERVO_G_CLOSE) {
        curGrip = SERVO_G_CLOSE;
